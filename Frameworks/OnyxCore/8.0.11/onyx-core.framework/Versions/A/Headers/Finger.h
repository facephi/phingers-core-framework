...
class Finger {
 public:
  ...
  /// This member function draws an ellipse on the given input mask
  /// \param a cv::Mat containing the input mask to modify
  void drawFingerMask(cv::Mat& mask) const {
  ...
  /// This member function returns the major axis length.
  /// \return the major axis length of the finger.
  float majorLength() const {
    return std::max(fingerEllipse.size.width, fingerEllipse.size.height);
  }

  /// This member function returns the minor axis length.
  /// \return the minor axis length of the finger.
  float minorLength() const {
    return std::min(fingerEllipse.size.width, fingerEllipse.size.height);
  }
